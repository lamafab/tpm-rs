...
pub fn cp_hash<CmdT: TpmCommand, D: Digest>(
    ...
    let n = cmd_handle.try_marshal(buf)?;
    hasher.update(&buf[..n]);
...
impl Session for HmacSession {
    ...
    fn validate_auth_response<CmdT: TpmCommand>(
        ...
        todo!()
        /*
        let computed_hmac = Self::_compute_hmac(
            &mut self.buf,
            cmd,
            resp_handles,
            &auth.nonce,
...
/// Compute session HMAC for an HMAC session.
pub fn compute_session_hmac<T: TpmCommand>(
    auth_key: &[u8],
    command: T,
    handle: TpmiRhHierarchy,
    nonce_caller: Tpm2bDigest,
    nonce_tpm: Tpm2bDigest,
    session: TpmaSession,
) -> Vec<u8> {
    ...
    let mut hasher = Sha256::new();
    {
        let n = T::CMD_CODE.try_marshal(&mut buf).unwrap();
        hasher.update(&buf[..n]);
        //
        let n = handle.try_marshal(&mut buf).unwrap();
        hasher.update(&buf[..n]);
        //
        let n = command.try_marshal(&mut buf).unwrap();
        hasher.update(&buf[..n]);
    }
    ...
    // Compute HMAC.
    let mut mac = Hmac::<Sha256>::new_from_slice(&auth_key).unwrap();
    {
        mac.update(&p_hash);
        //
        let n = nonce_caller.try_marshal(&mut buf).unwrap();
        mac.update(&buf[2..n]); // NOTE: excluding size indicator!
                                //
        let n = nonce_tpm.try_marshal(&mut buf).unwrap();
        mac.update(&buf[2..n]); // NOTE: excluding size indicator!
                                //
        let n = session.try_marshal(&mut buf).unwrap();
        mac.update(&buf[..n]);
    }
